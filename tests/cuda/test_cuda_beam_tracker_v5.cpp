...
//
// Unit tests and numerical validation for CUDA Beam Tracker V5.
// Validates:
// 1. Unified Single-Engine Warp Reduction (32, 64, 128, and 256 antennas)
// 2. Deep ILP Unrolling (TIME_UNROLL = 8, 4, 2)
// 3. Time-Chunk Tiling (80 vs 320)
// 4. Batched Continuous Execution & CUDA Graph Capture
// 5. Device-Resident Zero-Copy Execution
// 6. Pinned Memory Allocation Helpers
// 7. Partial final window remainder execution
// 8. Numerical parity against CPU Opt v2 reference.
...
bool run_test_configuration(const std::size_t n_ant, const std::size_t n_time = 3200, 
    ...
    std::printf("=================================================================\n");
    std::printf("Testing V5: n_ant=%zu, n_time=%zu, integration_spectra=%zu\n",
    ...
    // 2. V5 Unified Warp Reduction (Unroll 8, Chunk 80)
    {
        V5ExecutionConfig cfg;
        cfg.time_chunk_size = 80;
        cfg.time_unroll = 8;
        Intensities gpu_v5_u8(total_cells);
        cuda_beam_tracker_v5_into(packed, dims, tracker_cfg, gpu_v5_u8, cfg);
        if (!check_tolerance(cpu_ref, gpu_v5_u8)) {
            std::fprintf(stderr, "FAIL: V5 Unified (Unroll 8) failed for n_ant=%zu\n", n_ant);
            return false;
        }
        std::printf("  -> PASS: V5 Unified (Unroll 8, Chunk 80) matches CPU reference.\n");
    }

    // 3. V5 Unified Warp Reduction (Unroll 4, Chunk 80)
    {
        V5ExecutionConfig cfg;
        cfg.time_chunk_size = 80;
        cfg.time_unroll = 4;
        Intensities gpu_v5_u4(total_cells);
        cuda_beam_tracker_v5_into(packed, dims, tracker_cfg, gpu_v5_u4, cfg);
        if (!check_tolerance(cpu_ref, gpu_v5_u4)) {
            std::fprintf(stderr, "FAIL: V5 Unified (Unroll 4) failed for n_ant=%zu\n", n_ant);
            return false;
        }
        std::printf("  -> PASS: V5 Unified (Unroll 4, Chunk 80) matches CPU reference.\n");
    }

    // 4. V5 Unified Warp Reduction (Unroll 2, Chunk 320 - Single Warp per Channel)
    {
        V5ExecutionConfig cfg;
        cfg.time_chunk_size = 320;
        cfg.time_unroll = 2;
        Intensities gpu_v5_c320(total_cells);
        cuda_beam_tracker_v5_into(packed, dims, tracker_cfg, gpu_v5_c320, cfg);
        if (!check_tolerance(cpu_ref, gpu_v5_c320)) {
            std::fprintf(stderr, "FAIL: V5 Unified (Chunk 320) failed for n_ant=%zu\n", n_ant);
            return false;
        }
        std::printf("  -> PASS: V5 Unified (Chunk 320) matches CPU reference.\n");
    }

    // 5. BatchedTrackerStreamV5 (Standard Batch Execution)
    if (dims.n_time % tracker_cfg.integration_spectra == 0) {
        ...
        BatchedTrackerStreamV5 batched_stream(win_dims, tracker_cfg, window_count, cfg);
        ...
        if (!check_tolerance(cpu_ref, gpu_batched)) {
            std::fprintf(stderr, "FAIL: BatchedTrackerStreamV5 failed for n_ant=%zu\n", n_ant);
            return false;
        }
        std::printf("  -> PASS: BatchedTrackerStreamV5 matches CPU reference (Kernel time: %.3f ms).\n",
                    batched_stream.last_kernel_time_ms());
    }

    // 6. BatchedTrackerStreamV5 (CUDA Graph Execution)
    if (dims.n_time % tracker_cfg.integration_spectra == 0) {
        const std::size_t window_count = tracker_window_count(dims.n_time, tracker_cfg.integration_spectra);
        Dimensions win_dims{tracker_cfg.integration_spectra, dims.n_freq, dims.n_ant, dims.n_beams};
        V5ExecutionConfig cfg;
        ...
        BatchedTrackerStreamV5 graph_stream(win_dims, tracker_cfg, window_count, cfg);
        ...
        if (!check_tolerance(cpu_ref, gpu_graph)) {
            std::fprintf(stderr, "FAIL: BatchedTrackerStreamV5 (CUDA Graph) failed for n_ant=%zu\n", n_ant);
            return false;
        }
        std::printf("  -> PASS: BatchedTrackerStreamV5 with CUDA Graph matches CPU reference (Kernel time: %.3f ms).\n",
                    graph_stream.last_kernel_time_ms());
    }

    // 7. Device-Resident Zero-Copy Execution
    {
        ...
        if (!check_tolerance(cpu_ref, gpu_resident)) {
            std::fprintf(stderr, "FAIL: V5 Device-Resident execution failed for n_ant=%zu\n", n_ant);
            return false;
        }
        std::printf("  -> PASS: V5 Device-Resident execution matches CPU reference.\n");
    }

    // 8. Pinned Memory Allocation Test
    {
        auto pinned_v = allocate_pinned_voltage_v5(dims);
        auto pinned_i = allocate_pinned_intensities_v5(dims);
        ...
        cuda_beam_tracker_v5_into(pinned_packed_view, dims, tracker_cfg, gpu_pinned);

        if (!check_tolerance(cpu_ref, gpu_pinned)) {
            std::fprintf(stderr, "FAIL: V5 Pinned memory test failed for n_ant=%zu\n", n_ant);
            return false;
        }
        std::printf("  -> PASS: V5 Pinned memory allocators verified.\n");
    }
...
int main() {
    try {
        ...
        std::printf("\n=================================================================\n");
        std::printf("SUCCESS: All CUDA Beam Tracker V5 unit tests passed successfully!\n");
        std::printf("=================================================================\n");

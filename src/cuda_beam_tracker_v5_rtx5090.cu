...
//
// CUDA Beam Tracker V5 — Blackwell (RTX 5090) Architecture Edition.
//
// Optimized for: NVIDIA GeForce RTX 5090 / Blackwell Architecture (GB202 / SM 100/120)
// - 170 Streaming Multiprocessors (SMs) / 21,760 CUDA Cores
// - 1.79 TB/s GDDR7 Memory Bus (512-bit)
// - 128 MB High-Speed L2 Cache
// - Massive Register File & Dual-Issue FP32 Execution Pipeline
//
// Architectural Highlights:
// 1. Grid Sizing for 170 SMs:
//    Configured with 256 threads/block (8 warps per block) and high grid
//    sub-chunking to ensure full wave occupancy across all 170 SMs.
// 2. Deep 16-way and 8-way ILP unrolling (TIME_UNROLL = 16 / 8):
//    Keeps up to 16 time samples of accumulators in registers, saturating the
//    dual-issue FP32 pipeline and hiding instruction latency completely.
// 3. Non-Coherent Streaming Directives (`__ldg` / `ld.global.nc`):
//    Optimized for Blackwell's 128 MB L2 cache and 128-byte cache line sectors.
// 4. Hardware Bitfield Extraction (PTX `bfe.s32`):
//    Single-cycle sign-extended 4-bit integer decode into 32-bit registers.
// 5. Zero Shared Memory / Zero `__syncthreads()`:
//    Pure register accumulation and single 5-step warp shuffle reduction.

#include "beamformer/cuda_beam_tracker_v5_rtx5090.hpp"
...
void PinnedDeleterRTX5090::operator()(void* ptr) const noexcept {
    if (ptr != nullptr) {
        cudaFreeHost(ptr);
    }
}
...
PinnedVectorRTX5090<std::uint8_t> allocate_pinned_voltage_rtx5090(const Dimensions& dims) {
    ...
    return PinnedVectorRTX5090<std::uint8_t>(static_cast<std::uint8_t*>(ptr));
}
...
PinnedVectorRTX5090<float> allocate_pinned_intensities_rtx5090(const Dimensions& dims) {
    ...
    return PinnedVectorRTX5090<float>(static_cast<float*>(ptr));
}
...
template <int N_ANT>
__device__ __forceinline__ float3 tracker_position_rtx5090(const unsigned int element,
...
__device__ __forceinline__ void tracker_weight_rtx5090(const float3 position,
...
// ---------------------------------------------------------------------------
// Unified V5 Tracker Kernel Template (RTX 5090 / Blackwell Edition)
// ---------------------------------------------------------------------------
template <int N_ANT, int TIME_UNROLL>
__global__ void __launch_bounds__(256, 4)
tracker_v5_rtx5090_kernel(
    ...
    const float3 direction = make_float3(
        __ldg(&window_directions[window * 3 + 0]),
        __ldg(&window_directions[window * 3 + 1]),
        __ldg(&window_directions[window * 3 + 2]));
    const double wave_number = __ldg(&wavenumbers[freq]);

    // Precompute steering weights and pre-negated imaginary components in registers
    ...
    for (unsigned int a = 0; a < ANT_PER_LANE; ++a) {
        const float3 pos = tracker_position_rtx5090<N_ANT>(lane + a * 32U, spacing_m);
        tracker_weight_rtx5090(pos, direction, wave_number, &w_r[a], &w_i[a]);
        nw_i[a] = -w_i[a];
    }
    ...
    const std::uint8_t* __restrict__ packed_ptr =
        packed + (t_chunk_start * n_freq + freq) * N_ANT + lane;
    float* __restrict__ intensity_ptr =
        intensity + (t_chunk_start * n_freq + freq) * n_beams;

    std::size_t t = t_chunk_start;

    // Deep Unrolled Main Loop (ILP 16, 8, or 4)
    for (; t + (TIME_UNROLL - 1) < t_chunk_end; t += TIME_UNROLL) {
        ...
        for (unsigned int a = 0; a < ANT_PER_LANE; ++a) {
            ...
            for (int k = 0; k < TIME_UNROLL; ++k) {
                const std::uint32_t val = __ldg(&packed_ptr[k * t_stride + a_offset]);
                const float2 p = unpack_int4_fast(val);
                s_r[k] = fmaf(wra, p.x, fmaf(nwi, p.y, s_r[k]));
                s_i[k] = fmaf(wra, p.y, fmaf(wia, p.x, s_i[k]));
            }
        }

        // Intra-warp shuffle reduction tree
        #pragma unroll
    ...
    for (; t < t_chunk_end; ++t) {
        ...
        #pragma unroll
        for (unsigned int a = 0; a < ANT_PER_LANE; ++a) {
            const std::uint32_t val = __ldg(&packed_ptr[a * 32U]);
            const float2 p = unpack_int4_fast(val);
...
// ---------------------------------------------------------------------------
void host_pre_scan_rtx5090(const Dimensions& dims, const TrackerConfig& tracker,
...
// ---------------------------------------------------------------------------
// Unified Kernel Launch Dispatcher (RTX 5090 Edition)
// ---------------------------------------------------------------------------
void dispatch_rtx5090_kernel(
    float* d_intensity,
    ...
    const RTX5090ExecutionConfig& config,
    cudaStream_t stream) {
    ...
    constexpr unsigned int warps_per_block = 8; // 256 threads per block for 170 SM saturation
    ...
    auto launch_with_unroll = [&](auto n_ant_ic) {
        ...
        if (config.time_unroll >= 16) {
            tracker_v5_rtx5090_kernel<N_A, 16><<<grid_blocks, block_dim, 0, stream>>>(
        ...
        } else if (config.time_unroll >= 8) {
            tracker_v5_rtx5090_kernel<N_A, 8><<<grid_blocks, block_dim, 0, stream>>>(
                d_intensity, d_window_directions, d_wavenumbers, d_packed,
                dims.n_time, dims.n_freq, dims.n_beams,
                tracker.integration_spectra, chunk_size, chunks_per_window,
                spacing_m, total_warps);
        } else {
            tracker_v5_rtx5090_kernel<N_A, 4><<<grid_blocks, block_dim, 0, stream>>>(
                d_intensity, d_window_directions, d_wavenumbers, d_packed,
                dims.n_time, dims.n_freq, dims.n_beams,
                tracker.integration_spectra, chunk_size, chunks_per_window,
                spacing_m, total_warps);
...
void cuda_beam_tracker_v5_rtx5090_device_resident(
    ...
    std::vector<float> window_directions_flat;
    std::vector<double> wavenumbers;
    host_pre_scan_rtx5090(dims, tracker, window_directions_flat, wavenumbers);
    ...
    dispatch_rtx5090_kernel(d_intensity, d_window_directions, d_wavenumbers,
...
Intensities cuda_beam_tracker_v5_rtx5090(
    ...
    cuda_beam_tracker_v5_rtx5090_into(packed, dims, tracker, result, config);
    return result;
}

// ---------------------------------------------------------------------------
// Persistent Streaming Engine Implementation
// ---------------------------------------------------------------------------
struct BatchedTrackerStreamRTX5090::Impl {
    Dimensions win_dims;
    Dimensions batch_dims;
    TrackerConfig tracker_cfg;
    std::size_t batch_size;
    RTX5090ExecutionConfig config;
    ...
    Impl(const Dimensions& single_win_dims,
         const TrackerConfig& tracker,
         std::size_t b_size,
         const RTX5090ExecutionConfig& cfg)
        ...
        host_pre_scan_rtx5090(batch_dims, tracker_cfg, window_directions_flat, wavenumbers);
...
BatchedTrackerStreamRTX5090::BatchedTrackerStreamRTX5090(const Dimensions& single_window_dims,
                                                         const TrackerConfig& tracker,
                                                         std::size_t batch_size,
                                                         const RTX5090ExecutionConfig& config)
    : impl_(std::make_unique<Impl>(single_window_dims, tracker, batch_size, config)) {}

BatchedTrackerStreamRTX5090::~BatchedTrackerStreamRTX5090() = default;

void BatchedTrackerStreamRTX5090::process_batch(std::size_t first_window_index,
                                                const std::uint8_t* host_packed,
                                                float* host_intensity) {
    impl_->process_batch(first_window_index, host_packed, host_intensity);
}

void BatchedTrackerStreamRTX5090::process_batch_kernel_only(std::size_t first_window_index) {
    impl_->process_batch_kernel_only(first_window_index);
}

float BatchedTrackerStreamRTX5090::last_kernel_time_ms() const {
    return impl_->last_time_ms;
}

std::uint8_t* BatchedTrackerStreamRTX5090::device_packed_buffer() {
    return impl_->d_packed;
}

float* BatchedTrackerStreamRTX5090::device_intensity_buffer() {
    return impl_->d_intensity;
}

void* BatchedTrackerStreamRTX5090::device_stream() {
    return impl_->stream;
}

std::size_t BatchedTrackerStreamRTX5090::batch_size() const {
    return impl_->batch_size;
}

std::size_t BatchedTrackerStreamRTX5090::window_bytes() const {
    return voltage_sample_count(impl_->win_dims) * sizeof(std::uint8_t);
}

std::size_t BatchedTrackerStreamRTX5090::batch_voltage_bytes() const {
    return voltage_sample_count(impl_->batch_dims) * sizeof(std::uint8_t);
}

std::size_t BatchedTrackerStreamRTX5090::batch_output_floats() const {

// benchmarks/benchmark_cuda_tracker_v4.cpp
//
// Comprehensive Multi-Generation Benchmark Suite:
// CPU vs CUDA Legacy vs Phase 4 (FWS) vs V3 vs V4 (Deep ILP, Half2 SIMD, Graph).
//
// Evaluates:
// 1. CPU Naive, CPU Opt v1, CPU Opt v2
// 2. CUDA TwoPass, CUDA Fused, CUDA WarpReduction
// 3. CUDA Phase 4 Fused Warp Shuffle (FWS)
// 4. CUDA V3 Direct, CUDA V3 Batched Stream, CUDA V3 Batched Kernel Only
// 5. CUDA V4 Deep ILP (Unroll 4, Unroll 8)
// 6. CUDA V4 Half2 Vector SIMD
// 7. CUDA V4 Stream (Pipelined multi-stream)
// 8. CUDA V4 Batched Stream (Persistent buffers)
// 9. CUDA V4 Batched Graph (CUDA Graph accelerated)
// 10. CUDA V4 Batched Kernel Only (Compute bound)
// 11. CUDA V4 Device Resident (Zero-copy in-place)
...
void print_usage(const char* program) {
    std::cout
        ...
        << "  Benchmark CPU vs CUDA (Legacy, Phase 4 FWS, V3, V4) beam trackers.\n\n"
...
int main(int argc, char** argv) {
    try {
        ...
        std::cout << "================================================================================\n";
        std::cout << " CUDA Beam Tracker V4 Multi-Generation Benchmark Suite\n";
        ...
        if (dims.n_ant <= 64) {
...

            {
                ...
                auto stream = std::make_unique<beamformer::BatchedTrackerStreamV3>(win_dims, tracker_cfg, window_count, cfg);
                benchmark_engine("gpu", "v3_batched_stream", [&]() {
...

            {
                ...
                auto stream = std::make_unique<beamformer::BatchedTrackerStreamV3>(win_dims, tracker_cfg, window_count, cfg);
                stream->process_batch(0, packed.data(), output.data());
                benchmark_engine("gpu", "v3_batched_graph", [&]() {
                    stream->process_batch(0, packed.data(), output.data());
                });
            }

            {
                ...
                auto stream = std::make_unique<beamformer::BatchedTrackerStreamV3>(win_dims, tracker_cfg, window_count, cfg);
                stream->process_batch(0, packed.data(), output.data());
                benchmark_engine("gpu", "v3_batched_kernel_only", [&]() {
        ...
        // 5. CUDA V4
        if (dims.n_ant <= 64) {
            benchmark_engine("gpu", "v4_deep_ilp_u2", [&]() {
            ...
            benchmark_engine("gpu", "v4_deep_ilp_u4", [&]() {
                V4ExecutionConfig cfg;
                cfg.mode = V4KernelMode::DeepIlpWarpShuffle;
                cfg.time_unroll = 4;
                beamformer::cuda_beam_tracker_v4_into(packed, dims, tracker_cfg, output, cfg);
            ...
            benchmark_engine("gpu", "v4_half2_vector", [&]() {
                V4ExecutionConfig cfg;
                cfg.mode = V4KernelMode::Half2VectorSimd;
                beamformer::cuda_beam_tracker_v4_into(packed, dims, tracker_cfg, output, cfg);
            });
        } else {
            benchmark_engine("gpu", "v4_block_reduction_u2", [&]() {
                V4ExecutionConfig cfg;
            ...
            benchmark_engine("gpu", "v4_block_reduction_u4", [&]() {
                V4ExecutionConfig cfg;
                cfg.mode = V4KernelMode::BlockReduction;
                cfg.time_unroll = 4;
                beamformer::cuda_beam_tracker_v4_into(packed, dims, tracker_cfg, output, cfg);
        ...
        benchmark_engine("gpu", "v4_stream", [&]() {
            V4ExecutionConfig cfg;
            cfg.mode = V4KernelMode::Auto;
            cfg.time_unroll = 4;
            beamformer::cuda_beam_tracker_v4_stream(packed, dims, tracker_cfg, output, 3, cfg);
        });

        {
            ...
            auto stream = std::make_unique<beamformer::BatchedTrackerStreamV4>(win_dims, tracker_cfg, window_count, cfg);
            benchmark_engine("gpu", "v4_batched_stream", [&]() {
                stream->process_batch(0, packed.data(), output.data());
            });
        }

        {
            ...
            auto stream = std::make_unique<beamformer::BatchedTrackerStreamV4>(win_dims, tracker_cfg, window_count, cfg);
            stream->process_batch(0, packed.data(), output.data());
            benchmark_engine("gpu", "v4_batched_graph", [&]() {
                stream->process_batch(0, packed.data(), output.data());
            });
        }

        {
            ...
            auto stream = std::make_unique<beamformer::BatchedTrackerStreamV4>(win_dims, tracker_cfg, window_count, cfg);
            stream->process_batch(0, packed.data(), output.data());
            benchmark_engine("gpu", "v4_batched_kernel_only", [&]() {
                stream->process_batch_kernel_only(0);
            });
        }
        ...
        if (!has_p4) {
            p4_fws_ms = results[0].median_ms; // fallback
        }
        ...
        if (opts.outdir) {
            ...
            std::ofstream csv(*opts.outdir / "benchmark_cuda_tracker_v4_summary.csv");
            ...
            std::cout << "Wrote summary CSV to: " << (*opts.outdir / "benchmark_cuda_tracker_v4_summary.csv") << "\n";
        }
